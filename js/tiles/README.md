...
| `card: {name, collectible \| drop, count, need, status, statusCard, converted, positive, holdMs?}` | **blocking** card held on the board centre. The caption headlines `status` — what this copy just paid — and a **trophy** (`statusCard`) also counts itself `n of need`. Four holds: `cardHoldMs` a memory, `statusHoldMs` a trophy, `cardConvertMs` a trophy's third copy (with the big celebration), `clueHoldMs` a clue — which is also the only one that can be **held open**: tapping it stops the clock until Collect. `holdMs` overrides. `collectible` draws `cardFace()`, `drop` draws `dropFace()`; both are the faces the album and the box popup use |
