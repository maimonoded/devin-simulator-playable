"use strict";
/* Series-complete finale — state.seriesDone is already set by Tickets.award(),
   ...
   does, the run CONTINUES into it: coins, day, the card shoe and the unwatched episode queue
   all carry over and only the placeholders are fresh, because a series boundary is a chapter break in
...
function seriesComplete(){
  ...
  const totalEps=Tickets.totalEpisodes();
  const here=Tickets.series();
  const next=Economy.nextSeries();
  const acc=(()=>{const t=state.predWins+state.predLoss;return t?Math.round(state.predWins/t*100):0})();
  log("🏆",`<b>${(here&&here.name)||"SERIES"} COMPLETE.</b> ${totalEps} episodes in ${state.day-1} days.`);

  const cta=next
    ? `<button class="btn purple wide" id="nextSeries" style="margin-top:16px">Continue to ${next.name} →</button>
       <p class="hint" style="margin:8px 0 0;text-align:center">${next.episodes} more episodes · your coins, day and deck carry over</p>`
  ...
  $("#scrim").innerHTML=`<div class="modal"><div class="top"><div class="eyebrow">The finale</div><h2>${(here&&here.name)||"Series"} complete 🏆</h2></div>
    ...
    <div style="margin-top:8px;color:var(--muted)">All ${Tickets.count()} episodes collected in <b>${state.day-1}</b> days · ${state.epsWatched} watched · ${acc}% accuracy</div></div>
    ${cta}</div></div>`;
  $("#scrim").classList.add("show");

  if(next){
    $("#nextSeries").onclick=()=>{
      const s=Tickets.advanceSeries();
      $("#scrim").classList.remove("show");
      buildBoard(); renderAll();
      log("🎬",`<b>${s.name}</b> begins · episodes ${s.from}–${s.to}.`);
      toast(`🎬 ${s.name} — ${s.episodes} episodes`);
    };
  }else{
    $("#newSeries").onclick=()=>{
      $("#scrim").classList.remove("show");
      initState(); Economy.apply(); buildBoard(); renderAll();
      toast("🎬 New run — Day 1");
    };
  }
  renderAll();
