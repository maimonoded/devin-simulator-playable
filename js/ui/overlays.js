...
/* Finale — state.seriesDone is already set by applyUpgrade; this is pure celebration. */
function seriesComplete(){
  confetti(); setTimeout(confetti,300); setTimeout(confetti,600);
  const totalEps=cfg.buildings*cfg.tiers;
  log("🏆",`<b>SERIES COMPLETE.</b> ${totalEps} episodes across ${cfg.buildings} builders in ${state.day-1} days.`);
  $("#scrim").innerHTML=`<div class="modal"><div class="top"><div class="eyebrow">The finale</div><h2>Series complete 🏆</h2></div>
    ...
    <div style="margin-top:8px;color:var(--muted)">All ${cfg.buildings} builders maxed in <b>${state.day-1}</b> days · ${state.epsWatched} watched · ${(()=>{const t=state.predWins+state.predLoss;return t?Math.round(state.predWins/t*100):0})()}% accuracy</div></div>
